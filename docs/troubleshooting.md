...
## Gazebo 启动时概率性段错误（Segfault）

现象：`auto.sh` 启动后 Gazebo (gzserver) 崩溃退出（exit code 139），`controller_spawner` 超时失败，`junior_ctrl` 卡在等待 joint state feedback。

根因：`liblivox_laser_simulation.so`（Livox Mid360 自定义 Gazebo 插件）使用 ODE 多射线形状（`livox_ode_multiray_shape`）模拟 24000 点扫描，在 Gazebo 启动初期与其他插件并发初始化时存在资源竞争，导致概率性段错误。

验证方法：关闭 Livox 雷达后启动正常即可确认：

```bash
ENABLE_LIVOX=0 ./auto.sh  # 不会崩溃
```

解决方案：已将 Livox 自定义插件替换为 Gazebo 标准 `libgazebo_ros_laser.so`。标准插件发布 `sensor_msgs/LaserScan` 到 `/scan`，稳定且无段错误风险。建图节点直接消费 `/scan`，不再经过 `PointCloud` 中间转换。
