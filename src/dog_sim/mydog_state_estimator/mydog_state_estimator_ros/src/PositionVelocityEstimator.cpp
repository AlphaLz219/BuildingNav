...
void LinearKFPositionVelocityEstimator<T>::run() {
  T process_noise_pimu =
      this->_stateEstimatorData.parameters->imu_process_noise_position; //config/cheetah-3-defaults.yaml:imu_process_noise_position    :  0.02
  ...
  Vec3<T> a = this->_stateEstimatorData.result->aWorld + g; //control input
  // std::cout << "A WORLD\n" << a << "\n";
