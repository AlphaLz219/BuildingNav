...
def main():
    ...
    # 添加一个rosparam确认是否为仿真环境
    sim_param = rospy.get_param('~enable_pcd2livox', False)  # 私有参数
    if not sim_param:
        rospy.loginfo("Pointcloud2livox is enabled!")
        ...
        # 保持节点运行
        rospy.spin()
    rospy.loginfo("Pointcloud2livox is disabled!")
